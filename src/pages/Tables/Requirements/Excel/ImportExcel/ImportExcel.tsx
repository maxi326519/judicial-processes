  ...
  function dataConvert(data: any) {
    ...
    data.forEach((requirements: any, i: number) => {
      ...
      const currentData: RequirementsDetail = {
        ...
        remitenteGeneral: textParser(requirements[3] || ""),
        remitenteEspecifico: textParser(requirements[4] || ""),
        direccion: textParser(requirements[5] || ""),
        concepto: textParser(requirements[6] || ""),
        tipoProceso: textParser(requirements[7] || ""),
        numeroProceso: textParser(requirements[8] || ""),
        abogado: textParser(requirements[9] || ""),
        fechaVencimiento: newDate(requirements[10]),
        solicitudDadep: textParser(requirements[11] || ""),
        areaApoyo: textParser(requirements[12] || ""),
        solicitudConcepto: textParser(requirements[13] || ""),
        respuestaSolicitud: textParser(requirements[14] || ""),
        fechaRespuesta: newDate(requirements[15]),
        respuestaSipa: newDate(requirements[16]),
        estado: textParser(requirements[17] || ""),
        observaciones: textParser(requirements[18] || ""),
      };
    ...
    return {
      head: newData.map(
        (item: RequirementsDetail): RequirementsHeads => ({
          ...
          remitenteGeneral: item.remitenteGeneral,
          remitenteEspecifico: item.remitenteEspecifico,
        })
      ),
      details: newData,
    };
